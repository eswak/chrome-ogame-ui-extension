...
chrome.storage.local.get(null, function(items) {
  ...
  document.querySelector('#content').innerHTML = `
    <select id="select-config">
      ${Object.keys(window.configs).map((key) => {
        var config = window.configs[key];
        return '<option value="' + key + '" ' + (key == window.selectedConfig ? 'selected' : '') + '>' + '[' + config.universe.language.toUpperCase() + '] ' + config.universe.name + ' : ' + config.players[config.playerId].name + '</option>';
      }).join('')}
    </select>
...
function handleConfigContent(configKey) {
  ...
  wrapper.innerHTML += '<div><span style="color: #fff">- Days of score history (player) :</span> ' + Object.keys((config.history || {})[config.playerId] || {}).length + '</div>';
  wrapper.innerHTML += '<div><span style="color: #fff">- Days of score history (others) :</span> ' + Object.keys((config.history || {})['1'] || {}).length + '</div>';
  wrapper.innerHTML += '<div><span style="color: #fff">- Expedition results :</span> ' + Object.keys(config.expeditionResults || {}).length + '</div>';

  // exchange rate
  var exchangeRateSectionHtml = '<h3>Exchange rate :</h3>';
  exchangeRateSectionHtml += '<div style="color:#fff;margin:5px 0">Used to compute rentability times and "standard units".</div>';
  ...
  if (!config.features.shipatdock) {
    shipatdockThresholdHtml += '<div style="color:##F44336;margin:5px 0">The "ship at dock" feature is turned off, setting this threshold will not change anything in the interface, because the warning is not displayed anyway.</div>';
  }
  ...
  shipatdockThresholdHtml += [
    ...
    {
      v: '0.1',
      t: '10% of military points (' + Math.round(0.10 * militaryPoints * 1000) + ' - automatically updates)'
    },
    {
      v: '0.05',
    ...
    notificationKeys.forEach(function(key, i) {
      ...
      wrapper.innerHTML += `
        <div class="pending-notification" id="pending-notification-${i}">
          ...
          <span style="pointer-events:none" class="date">${getFormatedDate(notification.when, '[Y]-[m]-[d] [H]:[i]:[s]')}</span>
  ...
  featureSectionHtml += '<span class="feature-description">Add a little text to know how much time you have before hitting full storage.</span>';
  ...
  featureSectionHtml += '<span class="feature-description">Allow you to set reminders that will trigger a Google Chrome notification when missions arrive at destination or when a construction ends.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">Add a table on the alliance page where you can track the score and progress of your alliance members.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">Add a tab that allows you to compare your score progression against players of a similar score, or selected players.</span>';
  featureSectionHtml += '</div>';
  // Feature: stats: stats
  ...
  featureSectionHtml += '<span class="feature-description">On the stats page, display a list of resources on your planets, in flight, as well as your production and the time remaining before hitting full storage.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">On the stats page, display a chart of player scores progression over time.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">On the stats page, display a list of suggested next builds, based on the ROI.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">In the planet list, add little icon shortcuts on hover to transport resources or deploy fleet to a given planet.</span>';
  featureSectionHtml += '</div>';
  // Feature: shipatdock
  ...
  featureSectionHtml += '<span class="feature-description">In the planet list, add little icon to warn you if you have a large number of ships that are staying idle.</span>';
  featureSectionHtml += '</div>';
  // Feature: ship
  ...
  featureSectionHtml += '<span class="feature-description">On the fleet page, add shortcuts to small and large cargo, to allow to transport all the planet\'s held metal, crystal, deuterium, or any combination of them.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">On the fleet page, add text boxes to transport a given amount of metal, crystal, and deuterium, and allow to prefill the right number of small or large cargo to transport these resources.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-status ' + (config.features.expeditionpoints ? 'on' : 'off') + '"></span>';
  featureSectionHtml += '<span class="feature-name">Fleet page: expedition shortcut</span>';
  featureSectionHtml += '<span class="feature-description">On the fleet page, add a shortcut to send a fleet optimized for expedition based on the current server status (maximum resources available, player class, etc).</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">Add a tab that recaps the results of all past expeditions. You must read the expedition result messages in order to add them to the list.</span>';
  featureSectionHtml += '</div>';
  // Feature: galaxy
  ...
  featureSectionHtml += '<span class="feature-description">Add the score of the player and a link to all their planets in the galaxy view.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-description">In the galaxy view, add the number of resources contained in a debris field without having to hover it.</span>';
  featureSectionHtml += '</div>';
  // Feature: minetext
  ...
  featureSectionHtml += '<span class="feature-description">Add additional information on production buildings, to know the time to hit your return on investment.</span>';
  featureSectionHtml += '</div>';
  ...
  featureSectionHtml += '<span class="feature-status ' + (config.features.missingresources ? 'on' : 'off') + '"></span>';
  featureSectionHtml += '<span class="feature-name">Missing resources</span>';
  featureSectionHtml += '<span class="feature-description">Add the number of missing resources for constructions that are too expensive to start now.</span>';
  featureSectionHtml += '</div>';
  // Feature: solarsat
  ...
  featureSectionHtml += '<span class="feature-description">Pre-fill the number of solar satellite construction box with the number of satellites you need to build if your energy is negative.</span>';
  featureSectionHtml += '</div>';
  // Feature: topgeneral
  ...
  featureSectionHtml += '<span class="feature-description">In the top/flop tab, show the leaderboard of top gained/lost points.</span>';
  featureSectionHtml += '</div>';
  // Feature: topeco
  ...
  featureSectionHtml += '<span class="feature-description">In the top/flop tab, show the leaderboard of top gained/lost economy points.</span>';
  featureSectionHtml += '</div>';
  // Feature: topfleet
  ...
  featureSectionHtml += '<span class="feature-description">In the top/flop tab, show the leaderboard of top gained/lost fleet points.</span>';
  featureSectionHtml += '</div>';
  // Feature: topresearch
  ...
  featureSectionHtml += '<span class="feature-description">In the top/flop tab, show the leaderboard of top gained/lost research points.</span>';
  featureSectionHtml += '</div>';
