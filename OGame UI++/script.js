...
var userscript = function () {
  ...
  setInterval(function () {
    ...
    if (times.metal > 0 || times.crystal > 0 || times.deuterium > 0) {
      ...
      if (times[limitingreagent] > 0) {
        ...
        $('#content .production_info').append('<li class="enhancement">' + trad('ECONOMY_TIME', {
          time: prettyTime(totalPrice / totalProd)
        }) + '</li>');
        ...
        if (availableIn === 0) {
          ...
          $('#content .production_info').append('<li class="enhancement">' + trad('BUILDABLE_IN', {
            time: prettyTime(availableIn)
          }) + '</li>');
        }
      }
    }
    ...
    if ($('#resources_1_large:not(.enhanced)').length > 0) {
      ...
      $('#content .production_info').append('<li class="enhancement">' + trad('ROI', {
        time: prettyTime(rentabilityTime),
  ...
  var $statsEntry = $('<li class="stats enhanced"><span class="menu_icon"><div class="customMenuEntry2 menuImage empire"></div></span><a class="menubutton" href="#" accesskey="" target="_self"><span class="textlabel enhancement">' + trad('MENU_STATS') + '</span></a></li>');
  ...
  $neighboursEntry.click(function () {
    ...
    for (var i = 0; i < neighbours.length; i++) {
      ...
      $el.append($('<td class="tooltip js_hideTipOnMobile" title="' + trad('ECONOMY_SCORE_LONG', {
        noBold: true,
        scoreEco: neighbours[i].economyScore
      }) + '"><a href="?page=highscore&searchRelId=' + neighbours[i].id + '&category=1&type=1">' + prettyNumber(neighbours[i].economyScore) + '</a></td>'));
      $el.append($('<td class="tooltip js_hideTipOnMobile" title="' + trad('MILITARY_SCORE_LONG', {
        noBold: true,
        scoreMilitary: neighbours[i].militaryScore,
        ships: (neighbours[i].ships ? neighbours[i].ships : '0')
      }) + '"><a href="?page=highscore&searchRelId=' + neighbours[i].id + '&category=1&type=3">' + prettyNumber(neighbours[i].militaryScore) + ' (' + prettyNumber(neighbours[i].ships ? neighbours[i].ships : '0') + ')</a></td>'));
  ...
  var $simEntry = $('<li class="sim enhanced"><span class="menu_icon"><div class="customMenuEntry3 menuImage fleet1"></div></span><a class="menubutton" href="https://trashsim.universeview.be/" accesskey="" target="_blank"><span class="textlabel enhancement">' + trad('MENU_FIGHTSIM') + '</span></a></li>');
