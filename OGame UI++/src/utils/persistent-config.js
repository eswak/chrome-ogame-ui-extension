...
function saveConfigBuffer() {
  ...
  saveConfigTimeout = setTimeout(saveConfig, 500);
}
...
function saveConfig() {
  ...
  offthread(
    function (configStr) {
      ...
      return LZString.compress(configStr);
    },
    [configStr],
    function (compressed) {
      //console.log('saveconfig took', Date.now() - start, 'ms offthread. saved', Math.round(100 * (100 * (1 - compressed.length / configStr.length))) / 100, '% storage space (', configStr.length, 'char =>', compressed.length, 'char)');
      localStorage.setItem(configKey, compressed);
    }
  );
}

function getConfig() {
  var configStr = localStorage.getItem(configKey) || '{}';

  // migrate from raw JSON to zipped text
  if (configStr[0] === '{') {
    var start = Date.now();
    var compressed = LZString.compress(configStr);
    localStorage.setItem(configKey, compressed);
    configStr = compressed;
    //console.log('OGame UI++: Migration to zipped localstorage took', Date.now() - start, 'ms');
  }

  var start = Date.now();
  configStr = LZString.decompress(configStr);
  //console.log('unzip added', Date.now() - start, 'ms to load time');
