...
window.uipp_parseExpeditionMessage = function($el) {
  ...
  var strings = {
    fr: {
      darkMatterRegex: new RegExp(`L\`attaquant obtient (?<name>${_translate('UNIT_DM').replace(/(\(|\))/g, (_, p1) => `\\${p1}`)}) (?<amount>[^\\s]+)`, 'i'),
      darkMatterSize: {
        s: [
      ...
      resourceCrystalRegex: new RegExp(`L\`attaquant obtient (?<name>${_translate('UNIT_CRYSTAL')}) (?<amount>.+)`, 'i'),
      resourceDeuteriumRegex: new RegExp(`L\`attaquant obtient (?<name>${_translate('UNIT_DEUTERIUM')}) (?<amount>.+)`, 'i'),
      fleetSize: {
        s: [
    ...
    en: {
      darkMatterRegex: new RegExp(`(?<name>${_translate('UNIT_DM').replace(/(\(|\))/g, (_, p1) => `\\${p1}`)}) (?<amount>[^\\s]+) have been captured`, 'i'),
      darkMatterSize: {
        s: [
      ...
      resourceDeuteriumRegex: new RegExp(`(?<name>${_translate('UNIT_DEUTERIUM')}) (?<amount>.+) have been captured`, 'i'),
      fleetSize: {
        s: [
