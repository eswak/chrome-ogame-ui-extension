var fn = function () {
  ...
  window._addGalaxyDebrisInterval = function _addGalaxyDebrisInterval () {
    ...
    setInterval(function () {
	  ...
	  for (var i = 1; i <= 15; i++) {
		var $el = $('.js_debris' + i + ' [rel=debris' + i + ']:not(.enhanced');
		if ($el.length === 0) {
			continue;
		}
		...
		var style = 'position: absolute; top: -3px; font-size: 11px; left: 0; text-align: center; width: 30px; text-shadow: black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px, black 0 0 3px;';
		$el.append($('<div class="enhancement" style="' + style + '">' + uipp_scoreHumanReadable(debrisAmount) + '</div>'));
	  }
